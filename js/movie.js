...
function makeSave(save) {
    ...
    save.forEach(ele => {
        if(ele.img != 'null'){
            postIm = `<img src="https://image.tmdb.org/t/p/w500${ele.img}" onclick="switchPage(${ele.id}); makePage()">`;
        }
        else{
            postIm = `<img onclick="switchPage(${ele.id}); makePage()" src="img/noPost.png" alt="no poster img">`;
        }
        ...
        <div class="info">
            ...
            <span onclick="switchPage(${ele.id}); makePage()" class="name">${ele.title}</span>
        </div>
    </div>`
    });
